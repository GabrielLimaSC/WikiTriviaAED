...
carta *escolherCarta(carta *head) {
  ...
  carta *temp = malloc(sizeof(carta));
  temp = head;

  while (temp != NULL) {
    if (strcmp(novo->nome, temp->nome) == 0) {
      novo = escolherCarta(head);
    }
    temp = temp->prox;
  }
...
void creditos() {
  ...
  screenSetColor(YELLOW, BLACK);
  screenGotoxy(5, 18);
  printf("Feito por Eduardo Lins, Gabriel Lima, Maria Augusta e Vinicius "
         "Beltrão.");
  screenUpdate();
